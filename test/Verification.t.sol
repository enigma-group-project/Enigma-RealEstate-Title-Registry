...
contract VerificationTest is Test {
    ...
    function setUp() public {
        ...
        issuers.registerNotary(issuer);
        vm.prank(issuer);
        records.registerTitle(id, dataHash, "cid1", owner, "ipfs://meta");
    }

    function test_VerifyHashMatch() public view {
        assertTrue(verifier.verifyTitleHash(id, dataHash));
    }

    function test_WrongHashFails() public view {
        assertFalse(verifier.verifyTitleHash(id, keccak256("wrong")));
    }

    function test_FullVerifyActive() public view {
        (bool valid, IAttestationRegistry.Status status, address o, address i) = verifier.verifyTitle(id, dataHash);
        assertTrue(valid);
        assertEq(uint256(status), uint256(IAttestationRegistry.Status.Active));
        assertEq(o, owner);
        assertEq(i, issuer);
    }
    // TODO(slice-3): assert verifyTitle returns valid=false after a revoke.
}
