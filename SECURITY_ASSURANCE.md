# Security Assurance — Enigma-RealEstate-Title-Registry
...
| **[M] Mechanical** | Reentrancy protection on state-changers; access control enforced; CEI ordering; no unchecked external-call return; events on state change. | ✅ OZ `ReentrancyGuard` on `registerTitle` + AuditTrail mutators; OZ `Ownable` (Registrar) + Notary role; CEI; events on every mutation. |
...
- **Design & implement:** chose a registry/attestation model (not a token); built from reviewed OZ components (`Ownable`, `ReentrancyGuard`, `Pausable`); defined roles (Registrar, Notary, Owner), state (`Status`), and transfer/revoke/renew rules.
...
|---|---|---|
| Reentrancy (SC05) | `registerTitle`, AuditTrail mutators | OZ `ReentrancyGuard` + CEI (effects before any external call) |
...
| Fake issuer | `registerTitle` | gated by `IssuerRegistry.isNotary` (Registrar-granted) |
| Replay / duplicate record | `registerTitle` | `AlreadyExists` check on the record id |
| Denial via stuck state | issuance | `Pausable` lets the Registrar halt issuance during an incident, then `unpause` |
| Front-running | `revokeTitle` | issuer/Registrar-only; gas price does not change correctness |
