# 🧪 Build · Validate · Troubleshoot · Test — Enigma-RealEstate-Title-Registry

> **enigma-group-project** · student template (implement the TODOs)
> Repo: <https://github.com/enigma-group-project/Enigma-RealEstate-Title-Registry> · GUI: <https://enigma-group-project.github.io/Enigma-RealEstate-Title-Registry/> · Tracker: <https://enigma-group-project.github.io/Enigma-RealEstate-Title-Registry/tracker/>
...
|---|-------|----------|-----------------|---------------|---------------|
| 1 | Registration | `contracts/IssuerRegistry.sol` | `forge test --match-contract IssuerRegistryTest` | `frontend/src/modules/issuer/` | `registerNotary / deregisterNotary / transferRegistrar` |
| 2 | Title registration | `contracts/RecordRegistry.sol` | `forge test --match-contract RecordRegistryTest` | `frontend/src/modules/record/` | `registerTitle` |
| 3 | Verification | `contracts/Verification.sol` | `forge test --match-contract VerificationTest` | `frontend/src/modules/verification/` | `verifyTitle / verifyTitleHash` |
| 4 | Transfer / revoke / audit | `contracts/AuditTrail.sol` | `forge test --match-contract AuditTrailTest` | `frontend/src/modules/audit/` | `transferTitle / revokeTitle / updateEncumbrance / logTitleEvent` |
...
### A1. Get the code
```bash
git clone https://github.com/enigma-group-project/Enigma-RealEstate-Title-Registry.git
cd Enigma-RealEstate-Title-Registry
...
```bash
gh run list  -R enigma-group-project/Enigma-RealEstate-Title-Registry --workflow ci.yml --limit 1
gh run watch -R enigma-group-project/Enigma-RealEstate-Title-Registry
```

### B2. Troubleshoot remotely
Open the failed run → **build-test** → expand the red step to read the log, or:
```bash
gh run view -R enigma-group-project/Enigma-RealEstate-Title-Registry --log-failed
```
Same root causes as the table in **A4** (most often: forge-std install or a Solidity error).

### B3. Test the GUI on GitHub Pages
- 🌐 **https://enigma-group-project.github.io/Enigma-RealEstate-Title-Registry/** — the live four-module web app (published by `.github/workflows/pages.yml` from `frontend/src`).
- 📊 **https://enigma-group-project.github.io/Enigma-RealEstate-Title-Registry/tracker/** — the project task tracker.
...
- Run it on demand without a code change:
  ```bash
  gh workflow run "Auto-Grade" -R enigma-group-project/Enigma-RealEstate-Title-Registry     # or the Actions ▸ Auto-Grade ▸ "Run workflow" button
  ```
