...
- **If it were tokenized:** each `Title` is unique → a natural **ERC-721** (one NFT per certificate, `tokenId = id`), with the Registrar as minter and `Status` as on-chain metadata. The current design keeps it as a lean registry to avoid NFT transfer-market surface that the use case doesn't need.
