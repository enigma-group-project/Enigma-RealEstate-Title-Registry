...
/// @title RecordRegistry — Slice 2 (TitleRegistered  + state)  [STUDENT TEMPLATE]
...
contract RecordRegistry is IAttestationRegistry {
    ...
    /// @notice Create a record. onlyNotary + nonReentrant. Reject duplicate id (AlreadyExists). Emit TitleRegistered.
    function registerTitle(bytes32 id, bytes32 dataHash, string calldata cid, address owner_, string calldata metadataURI)
        external onlyNotary nonReentrant
    {
        // TODO(member2): if records[id].status != Status.None revert AlreadyExists();
        //               write the Attestation (status Active, issuedAt = block.timestamp); emit TitleRegistered.
