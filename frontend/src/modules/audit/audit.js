...
document.getElementById("transfer").onclick = async () => {
  // TODO(member4): await wc.audit.transferTitle(recordId(val("key")), val("newOwner"))
  out("TODO(member4): implement transferTitle call");
};
document.getElementById("revoke").onclick = async () => {
  // TODO(member4): await wc.audit.revokeTitle(recordId(val("key")), val("reason"))
  out("TODO(member4): implement revoke call");
};
document.getElementById("checkpoint").onclick = async () => {
  // TODO(member4): await wc.audit.logTitleEvent(recordId(val("key")), val("note"))
  out("TODO(member4): implement logTitleEvent call");
};
